...
export default function Sidebar({ state, width, height, sidebarComponents }) {
  ...
  let sorter = [
    ...
    { index: 1, condition: true, dom: <ProjectConfigurator state={state} /> },
    { index: 2, condition: true, dom: (state.get('toolBarSelected') == 1) ? null : <PanelLayerElements mode={state.mode} layers={state.scene.layers} selectedLayer={state.scene.selectedLayer} /> },
    ...
    { index: 7, condition: true, dom: (state.get('toolBarSelected') != 1) ? null : <CatalogList state={state} /> },
  ];
