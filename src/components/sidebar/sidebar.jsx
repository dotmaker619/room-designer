...
export default function Sidebar({ state, width, height, sidebarComponents }) {
  ...
  let sorter = [
    ...
    { index: 1, condition: true, dom: (state.get('toolBarSelected') == 3) ? null : <ProjectConfigurator state={state} /> },
    { index: 2, condition: true, dom: (state.get('toolBarSelected') != 2) ? null : <PanelLayerElements mode={state.mode} layers={state.scene.layers} selectedLayer={state.scene.selectedLayer} /> },
    ...
    { index: 7, condition: true, dom: (state.get('toolBarSelected') == 2) ? null : <CatalogList state={state} /> },
  ];
...
Sidebar.propTypes = {
  ...
  lotation: PropTypes.number.isRequired
};
